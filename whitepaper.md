...
### 3.3 Socialized PoW (Anti-Monopoly Bonus)
To prevent hashpower concentration and incentivize small nodes, $HOMEChain implements a **50/50 Socialized Reward Split**:
- **50% Finder Reward**: Goes to the validator who solves the block hash.
- **50% Bonus Pool**: Distributed among up to 100 active validators in a fair, round-robin queue.

This mechanism ensures that even nodes with low hashpower receive regular payouts, creating a "Universal Basic Reward" that keeps the network decentralized and makes mining viable for any consumer device.
